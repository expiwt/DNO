...
# DNO (Diffeomorphic Neural Operator) для уравнения Дарси на меняющейся геометрии

Репозиторий содержит пайплайн для масштабирования нейронных операторов (в духе FNO) на семейства 2D-доменов с разной геометрией и топологией, включая домены с внутренней границей (“дыркой”), через идею диффеоморфного отображения в универсальный домен.
...
- `test_hole.py`, `quad_test.py` — тесты/проверки/черновые эксперименты.
- `Loss_function.py` — функции потерь (например, относительная Lp-ошибка).
- `loss/` — сохранённые графики/визуализации лоссов.

Датасеты / генерация / маппинг:
- `sq_art_matlab/` — пайплайн без дырок (Python + MATLAB):
...
- `pentagon/` — пример/пайплайн для пятиугольников (CSV + OBJ + скрипты для построения карты).
...
Для “граничного эффекта” используется дополнительный канал `b` — рамка из единиц по краю и нули внутри (например, `np.pad(..., constant_values=1)`), чтобы модель видела границу в регулярном представлении.
...
- `NaN/Inf` заменяются на 0 в численных тензорах, но дырка остаётся “видимой” через отдельный канал.
...
2. **Построение отображения в универсальный домен с дыркой**:
   - универсальный домен: квадрат + круглая дырка фиксированного радиуса (например, 0.1) в центре,
...
### Базовый режим (без дырок)
- Архитектура: 2D FNO-подобный нейрооператор на регулярной сетке.
